const icons = {
    ...
    icon(path, type, title = null, useTooltipInstead = false) {
        if (type == icons.materialIconType) {
            return icons.materialIcon(path, title, useTooltipInstead);
        } else if (type == icons.heroIconType) {
            return icons.heroIcon(path, title, useTooltipInstead);
        }
    },
    ...
    dsToPathHtml(ds, type) {
        ...
        if (type == icons.materialIconType) {
            ...
            from = `<path stroke-linecap="round" stroke-linejoin="round">`;
        }

        for (let d of ds) {
            element = fromHTML(from);
